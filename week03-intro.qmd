...
view(mtcars)
tail(mtcars, 10) # preview first few lines, can designate how many
summary(mtcars)
?mtcars #opens help info
```
...
Models, miles per gallon, and horsepower
...
mean(mtcars$mpg) #the $ lets you select a variable/ column within df
```
...
mtcars <- mtcars |> 
  mutate(am_f = factor(am, labels. = c("auto", "manual")))
