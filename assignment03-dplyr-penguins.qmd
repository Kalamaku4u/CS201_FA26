...
relocato <- penguins
relocato <- relocato %>%
  relocate(individual_id, .before = species)
view(relocato)
```
...
```{r}
#!is.na(variable) instead of na.rm=true
```
